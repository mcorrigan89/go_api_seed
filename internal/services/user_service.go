...
func (service *UserService) GetUserByID(ctx context.Context, userId string) (*entities.User, error) {
	service.utils.logger.Info().Ctx(ctx).Str("userId", userId).Msg("Getting user by ID")
	userUUID, err := uuid.Parse(userId)
	if err != nil {
		service.utils.logger.Err(err).Ctx(ctx).Msg("Failed to parse user ID")
		return nil, err
	}

	user, err := service.userRepository.GetUserByID(ctx, userUUID)
	if err != nil {
